...
# memory-mapped I/O
GET_PAINT_BUCKETS = 0xffff00e4 #added by me
VELOCITY                = 0xffff0010
ANGLE                   = 0xffff0014
...
start_state:
	...
	la    $t6, puzzle #this is the puzzle pointer

	sw		$t6, REQUEST_PUZZLE #this is the request to get the puzzle

		check_for_ready:

			lw		$t7, ready
			bne		$t7, $zero, solve_it
			j		check_for_ready				# jump to check_for_ready

			solve_it:

				la		$a0, puzzle
				la		$a1, heap
				jal		copy_board				# jump to copy_board and save position to $ra

				la		$a0, heap
				move 	$a1, $zero
				move 	$a2, $zero
				la		$a3, puzzle
				jal		solve				# jump to solve and save position to $ra
				sw		$zero, ready

			if_solved:

				#beq		$t8, $zero, solve_it #if condition for starting to solve the puzzle
				la		$t6, puzzle
				la		$t9, SUBMIT_SOLUTION #submit the solved puzzle in this step nad next
				sw		$t6, 0($t9)
				lw		$s2, GET_PAINT_BUCKETS	#gets the paint buckets


			paint_and_move:

				blt		$s2, 100, start_state
				li		$s0, 1 		# $s0  = 1
				sw		$s0, ENABLE_PAINT_BRUSH ($0)
				li		$s1, 10
				sw		$s1, VELOCITY

			do_always:

				j		do_always				# jump to do_always
...
bonk_interrupt:
    ...
    #Fill in your code here
		...
		li		$t2, 90
		sw		$t2, ANGLE #for angle

		li		$t3, 0
		sw		$t3, ANGLE_CONTROL #for angle control

		li		$t4, 10
		sw		$t4, VELOCITY #for velocity
